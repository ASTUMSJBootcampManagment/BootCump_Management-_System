...
export default function Applications() {
  ...
  const load = async () => {
    ...
    try {
      const [applicationsResponse, batchesResponse] = await Promise.all([
        API.get("/admin/applications"),
        API.get("/batches"),
      ]);

      setApplications(getArray(applicationsResponse));
      setBatches(getArray(batchesResponse));
    } catch (error) {
      setToast({
  ...
  const approve = async () => {
    ...
    const batchId = selected.appliedBatch?._id || selected.appliedBatch;

    if (!batchId) {
      setToast({
        message: "This application has no batch. Select a batch before approval.",
        type: "error",
      });
      return;
    }
  ...
  const reject = async () => {
    ...
    try {
      ...
      setToast({
        message: response.data?.message || "Application rejected.",
        type: "success",
      });
  ...
  const filtered = applications.filter((application) => {
    ...
    const matchesSearch =
      ...
      application.universityId?.toLowerCase().includes(query);
  ...
  return (
    <>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-4 flex flex-col lg:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 outline-none focus:border-[#08c98b] text-sm"
            />
          </div>
          ...
          <button
            type="button"
            onClick={load}
            className="px-4 py-2.5 rounded-xl border border-slate-200 flex items-center justify-center gap-2 text-sm font-bold"
          >
            <RefreshCw size={15} />
            Refresh
          </button>
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50 border-b">
                  <tr>
                    ...
                    <th className="text-left px-5 py-4">University ID</th>
                ...
                <tbody className="divide-y">
                  {filtered.map((application) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        {application.appliedBatch?.name || "Not assigned"}
                      </td>
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4 mt-6">
              {[
                ...
                ["University ID", selected.universityId],
                ["Phone", selected.phoneNumber],
                ["Gender", selected.gender],
                ["Telegram", selected.telegramUsername],
                ["Codeforces", selected.codeforcesAccount],
                ["LeetCode", selected.leetcodeAccount],
                ["GitHub", selected.githubAccount],
                [
                  "Laptop",
                  selected.hasPersonalLaptop ? "Yes" : "No",
                ],
                [
                  "Internet",
                  selected.hasConstantInternet ? "Yes" : "No",
                ],
              ].map(([label, value]) => (
            ...
            {selected.applicationStatus === "waiting" && (
              <div className="flex justify-end gap-3 mt-6">
                <button
                  ...
                  className="px-4 py-2.5 rounded-xl bg-red-50 text-red-600 font-bold"
                >
                  Reject
                </button>
      ...
      {/* Reject Reason Modal */}
      {showReject && selected && (
        <div className="fixed inset-0 z-[110] bg-slate-900/40 grid place-items-center p-5">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md">
            ...
            <textarea
              ...
              className="w-full mt-4 border border-slate-200 rounded-xl p-3 text-sm outline-none focus:border-[#08c98b]"
            />
            ...
            <div className="flex justify-end gap-3 mt-4">
              ...
              <button
                ...
                onClick={reject}
                disabled={processing}
                className="px-4 py-2.5 rounded-xl bg-red-600 text-white font-bold disabled:opacity-50"
              >
